# 🦠 COVID-19 Global Analytics Pipeline

> An end-to-end data engineering and analytics project built with **PySpark**, analyzing global COVID-19 trends across 6 real-world datasets — covering 180+ countries, daily time-series, WHO regions, US county-level data, and population statistics.
...
- **`HADOOP_HOME` error on Windows** — See [Setup section](#️-windows-users--winutils-required) above
- **Country name mismatches** — The standardization map in Task 3 covers the most common variants; extend the `mapping` dict for additional edge cases
- **`usa_county_wise.csv`** — Country identifier is `US`, not `USA`; this is handled in Task 3's standardization
- **Parquet output** — Each write creates a directory with part files, not a single file. This is normal Spark behavior. Use `.coalesce(1)` before `.write` if a single file is needed
- **Schema inference** — Some numeric columns may infer as strings if the CSV has mixed values; cast explicitly if aggregations fail
- **`day_wise.csv` column names** — Contain spaces; access via the renamed aliases (`Dper100`, `Rper100`, `DRper100`) after `load_datasets()` runs

---

## Author

Built as part of an internship data engineering project.  
Dataset credit: [Devakumar kp — Kaggle](https://www.kaggle.com/datasets/imdevskp/corona-virus-report)
